...
void EffectiveSink::Log(const LogMsg& msg) {
  ...
  }
  {
    ...
    MemoryBuf encrypted_temp_buf;
    encrypted_temp_buf.reserve(buf.size() + 16);
    crypt_->Encrypt(buf.data(), buf.size(), encrypted_temp_buf);
    if (encrypted_temp_buf.empty()) {
      LOG_ERROR("EffectiveSink::Log: encrypt failed");
      return;
    }

    size_t encrypted_size = encrypted_temp_buf.size();
    compressed_buf_.reserve(compress_->CompressedBound(encrypted_size));

    size_t compressed_size =
        compress_->Compress(encrypted_temp_buf.data(), encrypted_size, 
    ...
    WriteToCache_(compressed_buf_.data(), compressed_size);
  }
